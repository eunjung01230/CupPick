...
> 문서: 06-data.md · 버전: 1.4 · 갱신: 2026-09-18
> 기준: 01~04의 현재 MVP · 05-policy.md v1.4 · 최종 CupPick v2 디자인
...
- 단독 D번호는 이 파일의 D01~D22. 00-backlog.md의 위임결정 D번호는 `00-D01`처럼 문서명을 붙인다. 정책은 POL01~POL26이다.
...
| D04 | 브랜드·프로그램·잔액 분류 | 공통 서버 기준정보 | F03·F06·F08·F12·F15·F19·F22·F25·F34 | POL01·POL06·POL24·POL26 |
...
| 칸 | 형식·필수 여부 | 규칙·예시 |
| --- | --- | --- |
| 계정ID | 고정 ID·필수 | 개인 데이터 소유자. 모든 개인 자료가 참조하는 단일 주체이며 인증 수단과 분리한다 |
| 소셜 로그인 계정 | 자식행: 로그인 제공자+제공자 사용자ID+제공자 이메일·최소 1개 | 구글/네이버/카카오와 해당 subject 조합이 전체에서 고유. 한 계정이 여러 자식행을 가질 수 있다. 제공자 이메일은 표시·문의 참고용이며 식별·병합 키로 쓰지 않음 |
| 인증한 휴대폰 번호·인증 시각 | 식별자+시각·선택 | 사용자가 직접 본인인증을 마친 경우에만 값이 있다. 계정 연결 판단의 유일한 근거이며 현재 범위에서는 만들지 않음(POL02.5) |
| 계정 상태·가입시각 | 활성/탈퇴처리중+시각·필수 | 탈퇴 확정 시 즉시 접근 차단 |
| 탈퇴 요청·운영삭제 기한 | 시각·탈퇴 시 필수 | 요청+24시간. 삭제완료 상태는D21로 최소 추적 |

수명·경계: 계정 유지 중 보관. 탈퇴 후 운영데이터 24시간, 백업 30일. 공식 브랜드 앱 비밀번호·결제카드·불필요한 프로필은 저장하지 않는다. 소셜 로그인 계정 자식행은 계정 삭제 시 함께 제거한다. 현재 범위는 로그인마다 자식행 1개인 새 계정을 만들고 기존 계정에 자식행을 추가하지 않는다. 자식행을 여럿 두는 것은 POL02.5의 연결 조건을 만족할 때뿐이며, 인증한 휴대폰 번호는 본인인증 기능을 도입할 때만 채운다.
...
| 저장한 장소 | 자식행: 이름+좌표/지역표시+표시 순서·선택 | 계정당 최대 10개. 이름·위치·순서는 저장 장소 관리 화면에서만 변경한다. 방문·조회 이력이 아니며 자동으로 쌓지 않는다. 삭제해도 그때의 조회 좌표는 D03 마지막 조회 위치에 일회성 지정 위치로 남고 이 목록에서만 빠진다 |

수명·경계: 개인설정은 계정 수명. 마지막 위치·화면은 기기 복원 목적, 다른계정에 개인 기준이 노출되지 않게 분리한다. 위치 이동이력을 서버에 누적하지 않는다. 저장한 장소는 계정 단위로 최대 10개를 서버 제약으로 검사하며, 삭제는 그 자식행만 제거하고 마지막 조회 위치 좌표를 함께 지우지 않는다.

### D04. 브랜드·프로그램·잔액 분류

저장: 공통 서버 기준정보 · 정책: POL01·POL06·POL24·POL26

| 칸 | 형식·필수 여부 | 규칙·예시 |
| --- | --- | --- |
| 브랜드ID·표시이름 | ID+문자열·필수 | 1차 대상 7개(05 POL26). 예: 매머드 |
| 프로그램ID·상위브랜드ID·이름 | ID+참조+문자열·필수 | 예: 매머드커피/매머드익스프레스. 같은 브랜드 안 별도 프로그램 |
| 프로그램 유형 | 누적형/매장 조건형/단계 교환형·필수 | 교환·판정 분기의 기준. 미확인 프로그램은 유형 없음으로 두고 자동 계산하지 않음 |
| 교환 기준 수량 | 정수 또는 단계 목록·확인된 경우 | 누적형·매장 조건형은 단일 값, 단계 교환형은 오름차순 단계 목록과 단계별 혜택 이름. 예: 12·20 |
| 최소 매장 적립 수 | 정수·매장 조건형일 때 | 그 매장에서 쿠폰을 쓰려면 필요한 적립 수. 확인된 값만 사용 |
| 매장별 분리 여부 | 참/거짓·필수 | 참이면 매장마다 별도 활성 잔액. 서로 다른 매장 수량을 합산하지 않음 |
| 용도·적립범위 선택정의 | 분류값·검증된 경우 | 교환용/승급용 등, 공통/지점. 실제 정책 확인 없이 자동적용 금지 |
| 기준정보 상태·근거정책ID | 확인/미확인/보류+참조 | 미확인 브랜드도 수동기록은 허용 |

수명·경계: 활성 서비스 기준정보로 보관. 이름변경은ID 변경이 아니며, 기존개인 기록 참조를 유지한다. 유형·교환 기준·최소 매장 적립 수는 POL26에 확정된 범위만 넣고, 확인되지 않은 브랜드 수치를 추정해 자동 규칙으로 넣지 않는다. 브랜드 추가는 후순위이므로 이 표를 임의로 늘리지 않는다.
...
| 매장별 적립 수량 | 자식행: 매장참조 또는 직접입력이름+정수≥0·매장 조건형일 때 | 하나의 잔액 안에서 매장마다 몇 개 쌓였는지. 매장별 합계=현재 기록수량이어야 저장. 적립 매장을 모르는 수량은 행을 만들지 않고 합계 차이로 남김 |
...
매장별 적립 수량은 D08 묶음과 **다른 축**이다. D08은 만료일·구성 단위이고 이 자식행은 매장 단위 집계다. 같은 스탬프를 두 축에서 이중으로 세지 않는다. 단계 교환형 중 매장별 분리 프로그램(매머드커피)은 이 자식행 대신 매장마다 별도 D07 잔액(적립범위=특정지점)으로 관리한다.
...
| 발급 시각·사용 시작 시점·조건문구 | 시각/날짜+시각/문자열·선택 | `issuedAt`과 `usableFrom`은 별도 값이며 서로 대체하지 않는다. 등록일≠발급일. `usableFrom`은 날짜 단위(빽다방: 발급 다음 날)와 분 단위(메가MGC: `issuedAt`+5분)를 모두 담아야 하므로 분 단위 대기 브랜드는 시각까지 보관한다. 확인된 정책이 없는 브랜드는 값 없이 조건문구만. 메뉴·주문 조건은 보조문구 |
| 사용 가능 매장 보존 상태 | D10 조건 참조·매장 조건형 쿠폰일 때 | 쿠폰을 기록한 시점의 매장별 적립 수량 사본. 이후 적립으로 갱신하지 않는다 |
...
| 보존 기준·보존 시각 | 기록시점스냅샷/현재상태+시각·매장 조건형일 때 | 스냅샷이면 그 시각의 매장별 적립 수량으로 고정한다 |
| 항목별 보존 적립 수량·필요 수량 | 자식행 정수≥0+정수·보존 스냅샷일 때 | 매장마다 당시 적립 수와 최소 필요 수를 함께 남겨 `적립 N/3`을 표시할 수 있게 한다 |

수명·경계: 부모 수명과동일. 일부목록의 바깥 모든 매장을불가행으로 저장하지 않는다. 공통 정책과개인 확인 기록을별도 보관하고표시판정은 조합계산한다.

**매장 조건형 쿠폰의 보존 규칙(POL14.5·POL14.6).** 쿠폰을 기록할 때 그 시점의 D07 매장별 적립 수량을 이 조건에 복사해 고정한다. 이후 D07의 매장별 수량이 늘어도 **기존 쿠폰의 조건을 다시 계산하지 않는다.** 판정은 보존 수량만 사용한다. 보존 수량이 최소 필요 수 이상이면 사용 가능, 미만이면 사용 불가와 부족 수량, 보존 목록에 없는 매장은 미확인이다. 보존 상태 자체가 없으면 그 쿠폰의 매장 조건은 전체 미확인이다. 기록 후의 적립은 다음 쿠폰을 위한 D07 상태로만 쌓고 기존 쿠폰의 보존 상태와 섞지 않는다.
...
| D04 | 프로그램 유형(누적형/매장 조건형/단계 교환형)·교환 기준 수량 또는 단계 목록·최소 매장 적립 수·매장별 분리 여부. 값은 05 POL26 범위만. 미확인 프로그램은 유형 없음으로 두고 자동 계산 대상에서 제외 |
| D06 | 매장의 프로그램 연결을 확인된 경우 필수로 채운다. 매머드 매장은 매머드커피/매머드익스프레스 중 어느 프로그램인지 구분한다. 프로그램 미확인 매장을 특정 프로그램의 적립·사용 후보로 확정하지 않는다 |
| D07 | 매장 조건형 잔액의 매장별 적립 수량 자식행. 매장별 합계=현재 기록수량 제약. D08 묶음과 별도 축이며 이중 계산 금지. 매장별 분리 프로그램은 자식행 대신 매장별 별도 잔액 |
| D09 | `issuedAt`·`usableFrom` 분리 보관. 매장 조건형 쿠폰은 기록 시점 매장 상태를 가리키는 D10 보존 조건 참조를 갖는다. 표시 상태는 저장하지 않고 계산한다 |
| D10 | 보존 기준(기록시점스냅샷/현재상태)·보존 시각·항목별 보존 적립 수량·필요 수량. 스냅샷은 생성 후 D07 변화로 갱신하지 않는다 |
| D11 | 전환 기록 사건에 교환 단계(단계 교환형)와 보존한 매장 상태 참조를 남긴다. 사건 재시도 시 같은 보존 상태를 재사용하고 새로 계산하지 않는다 |
...
9. 매장 조건형 잔액의 매장별 적립 수량 합계는 그 잔액의 현재 수량과 같다. 어긋난 상태로 저장하지 않는다. 매장별 수량과 D08 묶음 수량을 더해 하나의 잔액으로 합산하지 않는다.
10. 매장 조건형 쿠폰의 D10 보존 스냅샷은 기록 시점 이후 D07 변화에 영향받지 않는다. 새 적립은 D07에만 반영하고 이미 기록한 쿠폰의 보존 수량을 수정하지 않는다.
11. 같은 상위 브랜드라도 프로그램이 다르면 별도 잔액·별도 쿠폰이다. 매머드익스프레스와 매머드커피, 매머드커피의 서로 다른 매장 잔액을 하나의 수량으로 합치지 않는다.
12. 하나의 소셜 로그인 계정(제공자+제공자 사용자ID)은 하나의 사용자 계정에만 속한다. 같은 조합을 두 계정이 나눠 갖지 않으며 이 고유성은 저장 시점에 강제한다. 계정에는 소셜 로그인 계정이 최소 1개 있어야 한다. 이메일·이름·프로필은 계정을 식별하거나 합치는 키가 아니다. 인증한 휴대폰 번호가 같다는 사실만으로 저장된 계정이 자동으로 합쳐지지 않는다. 연결은 사용자가 직접 요청하고 본인인증을 마친 경우에만 일어난다(POL02.2·POL02.5).
...
| 매장 최종가능표시 | D05공통 규칙+D10개인근거/완전성/충돌+D06연결. 매장 조건형은 D10의 보존 스냅샷만 사용하고 현재 D07 매장별 수량으로 재계산하지 않는다. 근거없는확정캐시금지 |
| 쿠폰 표시 상태 | D09 생애+날짜 신뢰+`usableFrom`과 실행 시점의 실제 한국 시각. 기록됨/아직 사용 불가/사용 가능/만료 임박/사용 완료/만료를 계산하며 저장하지 않는다 |
| 교환 가능 여부·다음 단계까지 남은 수 | D07 현재 수량+D04 교환 기준. 단계 교환형은 도달한 단계와 다음 단계를 함께 계산하고 자동 전환하지 않는다 |
| 매장별 적립 미확인 수량 | D07 현재 수량−매장별 적립 수량 합. 0이어야 정상이며 차이는 적립 매장 미확인 수량이다 |
...
| 저장한 장소 남은 자리 | 10−D03 저장한 장소 자식행 수. 저장 요청 시 서버에서 다시 검사한다 |
...
| 매장 조건형 총8을 4/3/1로 등록 | D07 현재수량 8, 매장별 자식행 3개 합계 8. 합계가 8이 아니면 저장 거부 |
| 위 상태에서 쿠폰 기록 | D09 쿠폰 생성, D10에 4/3/1과 필요 수 3을 보존. 4·3 매장은 사용 가능, 1 매장은 `적립 1/3` 사용 불가 |
| 쿠폰 기록 후 1 매장에 2개 추가 적립 | D07 해당 매장 3으로 증가, D10 보존값은 1 유지. 기존 쿠폰 판정 불변, 다음 쿠폰부터 3으로 반영 |
| 매머드커피 두 매장 12·5 보유 | 매장별 D07 잔액 2개. 12 매장만 교환 가능, 합산 17로 표시하지 않음 |
| 매머드커피 12개 교환 기록 | 해당 매장 D07만 12 차감, D09 아메리카노 쿠폰 1장, D11에 교환 단계 12 기록. 다른 매장·익스프레스 잔액 불변 |
| 발급 다음 날부터 사용 가능한 쿠폰 | D09 `issuedAt`=오늘, `usableFrom`=내일. 오늘은 표시 상태 `아직 사용 불가`, 만료 계산은 별도 |
| 메가MGC 리워드 쿠폰 발급 2분 경과 | D09 `issuedAt`=발급 시각, `usableFrom`=`issuedAt`+5분. 표시 상태 `아직 사용 불가`·`3분 후 사용 가능`. 5분 경과 후 `사용 가능` |
| 조회 기준으로 쓰던 저장 장소 삭제 | D03 저장한 장소 자식행만 제거. 마지막 조회 위치 좌표는 일회성 지정 위치로 유지하고 목록을 다시 조회하지 않음 |
| 저장한 장소 10개에서 추가 저장 | 저장 거부·정리 안내. 기존 항목을 덮어쓰지 않음 |
...
| F06 | POL01·POL05·POL26 | D03·D04·D07·D08·D09·D13·D20 |
| F07 | POL05 | D03·D04·D07·D08·D09·D13 |
| F08 | POL06·POL26 | D04·D06·D07·D08·D11·D19 |
| F09 | POL06 | D07·D08·D11·D12·D13·D17·D19 |
| F10 | POL07·POL26 | D07·D08·D11·D12·D13·D17·D19 |
...
| F15 | POL11·POL26 | D04·D05·D07·D08·D09·D10·D11·D12·D13·D17·D19 |
...
| F22 | POL14·POL26 | D04·D05·D06·D07·D09·D10·D11·D13·D19 |
| F23 | POL14 | D05·D07·D08·D09·D10·D11·D13·D17·D19 |
| F24 | POL15·POL24 | D06·D10·D11·D19·D22 |
| F25 | POL01·POL15·POL26 | D04·D05·D06·D07·D08·D09·D10·D13·D20 |
...
1차 지원 브랜드 7개와 프로그램 유형·교환 기준·최소 매장 적립 수는 05 POL26이 원본이다. 이 파일은 그 값을 저장할 자리만 정의하고 값을 새로 만들지 않는다.
