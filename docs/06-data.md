...
> 문서: 06-data.md · 버전: 1.2 · 갱신: 2026-09-17
> 기준: 01~04의 현재 MVP · 05-policy.md v1.2 · 최종 CupPick v2 디자인
...
| 저장한 장소 | 이름+좌표/지역표시 목록·선택 | ‘자주 가는 곳’으로 바로 선택하는 조회 위치. `[?]` 개수 상한·편집 방법 미확정. 방문 이력이 아니며 자동으로 쌓지 않는다 |
...
| 발급일·사용시작일·조건문구 | 날짜/문자열·선택 | `issuedAt`과 `usableFrom`은 별도 값이며 서로 대체하지 않는다. 등록일≠발급일. 발급 후 분 단위 대기는 확인된 값이 있을 때만 저장하고 없으면 조건문구. 메뉴·주문 조건은 보조문구 |
