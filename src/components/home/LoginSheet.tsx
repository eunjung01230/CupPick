 ...
 * 로그인 유도 시트.
 *
 * 개인 데이터가 필요한 기능에 들어갈 때만 뜬다(docs/05-policy.md POL02.1 ·
...
export function LoginSheet({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    ...
    >
      <div className={styles.sheet} role="dialog" aria-modal="true" aria-labelledby="login-sheet-title">
        ...
        <div className={styles.sheetActions}>
          {SOCIAL_LOGINS.map((social) => (
            <form action={startSocialLogin} key={social.provider}>
              <input type="hidden" name="provider" value={social.provider} />
              {/* 로그인 성공은 요청한 화면(홈)으로 복귀한다(F04). */}
